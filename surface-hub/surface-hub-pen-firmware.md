...
This section describes how to update pen firmware via the automated maintenance cycles for Windows Update, which is configured by default to occur nightly at 3 a.m. You'll need to plan for two maintenance cycles to complete before applying the update to the Surface Hub Pen. Like other updates, pen firmware can be deployed using Windows Update for Business (WUfB). To learn more, see [Managing Windows updates on Surface Hub](manage-windows-updates-for-surface-hub.md).

1. Ensure the Surface Hub Pen is paired with Surface Hub: Press and hold the **top** button until the white indicator LED light begins to blink.
...
3. Select the pen to complete the pairing process.
4. Ensure only one Surface Hub Pen is connected at a time. Disconnect all other pens before starting the update, as the tool can update only one pen per session.
...
2. The tool reports the firmware version of the pen.
...
2. Double click **PenCfu.exe** to initiate the firmware update. The configuration and firmware image files must be stored in the same folder as the tool.
3. For more options, run **PenCfu.exe -h** to display the available parameters listed in the following table. Example: `PenCfu.exe -h`
