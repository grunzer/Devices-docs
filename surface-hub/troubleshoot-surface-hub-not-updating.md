...
description: Troubleshoot Surface Hub update issues, including proxy settings, WSUS conflicts, deferral policies, and nightly maintenance failures.
...
First, determine the OS build number that the Surface Hub is currently running. You can do this in-person via **Settings > Surface Hub > About**.
...
For troubleshooting purposes you can test connecting the Surface Hub to an open network to see if the device can download Windows Updates. This will help confirm if the internal network isn't meeting the necessary network requirements for Windows Update. To test with an open network do the following:
